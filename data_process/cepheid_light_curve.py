...
class CepheidLightCurve(ASASLightCurve):
    def run(self):
        ...
        if self.meta["period"] is None:
            ...
            if self.meta["main_main_type"] == "eclipsing_binary":
    ...
    def deep_run(self, time, measurements, period, epoch_hjd):
        ...
        self.meta["peri_optical"] = self.get_peri_optical(folded_flux, period)
        self.meta["pearsonr"] = self.get_pearsonr(folded_flux)
        self.meta["Mm_period_ratio"] = self.get_Mm_period_ratio(folded_time, folded_flux, period)
        self.meta["l1"] = self.get_l1_autocorrelation(folded_flux, folded_flux_err)
    ...
    @staticmethod
    def get_l1_autocorrelation(mag, err):
    ...
    @staticmethod
    def get_peri_optical(mag, period):
        return mean(mag) / period

    @staticmethod
    def get_pearsonr(mag):
        y1 = mag
        y2 = mag[::-1]
        return pearsonr(y1, y2)[0]

    @staticmethod
    def get_Mm_period_ratio(t, y, period):
        t_max_idx = t[np.argmax(y)]
        t_min_idx = t[np.argmin(y)]
        p1 = abs(t_max_idx - t_min_idx)
        p2 = abs((t[-1] - max(t_min_idx, t_max_idx))) + abs((t[0] + min(t_min_idx, t_max_idx)))
        return min(p1, p2) / period
